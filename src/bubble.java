...
public class bubble {
    ...
    public bubble(int xPos, int yPos, PApplet c){ //Setting up the variables
        ...
        //color = canvas.color(canvas.random(255),canvas.random(255),canvas.random(255)); //Random color for ball
        health = 3;
    ...
    public void update(){ //makes bubble move
        x += speed;
        if (x + size/2 > canvas.width || x - size/2 < 0){ // Makes so ball doesn't go of the screen when touches the wall
            speed = -speed;
           // color = randomColor();
            health--;
        }
